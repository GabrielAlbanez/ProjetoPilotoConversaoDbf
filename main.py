...
def mostrar_modal(clinica):
    ...
    # Remove as aspas ao redor do nome do convênio
...
def main():
    ...
    label_titulo = tk.Label(root, text="Selecione um Convênio", font=("Arial", 14, "bold"), bg="#2e2e2e", fg="white")
    label_titulo.pack(pady=20)

    global combobox
    combobox = ttk.Combobox(root, values=config["Convenios"], font=("Arial", 12), state="readonly")
    combobox.set("Escolha um convênio")
    combobox.pack(pady=10, padx=20)

    botao = ttk.Button(root, text="Selecionar", command=mostrar_selecao)
